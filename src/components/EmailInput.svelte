...
<div class="w-full relative">
	<div class="border-b border-[rgb(63_67_99)] text-[#797EA3] focus-within:border-white focus-within:text-white transition-colors duration-300">
		<label for='email' class="font-light text-[15px]">{label} {required ? '*' : ''}</label>
		<input id='email' type="text" bind:value class="w-full outline-none bg-inherit caret-white text-white" required={required}>
	</div>
	{#if error}
